...
abstract class SchemaParser {
    ...
    private Map<String, UserType> buildUserTypes(KeyspaceMetadata keyspace, List<Row> udtRows, VersionNumber cassandraVersion, Cluster cluster) {
        ...
        if (udtRows != null) {
            for (Row udtRow : udtRows) {
                UserType type = UserType.build(keyspace, udtRow, cassandraVersion, cluster);
    ...
    static Map<String, Map<String, Map<String, ColumnMetadata.Raw>>> groupByKeyspaceAndCf(ResultSet rs, VersionNumber cassandraVersion, String tableName, Cluster cluster) {
        ...
        for (Row row : rs) {
            ...
            ColumnMetadata.Raw c = ColumnMetadata.Raw.fromRow(row, cassandraVersion, cluster, ksName);
            l.put(c.name, c);
        }
        return result;
    }
