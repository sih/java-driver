...
public class FunctionMetadata {
    ...
    static FunctionMetadata build(KeyspaceMetadata ksm, Row row, VersionNumber version, Cluster cluster) {
        ...
        Map<String, DataType> arguments = buildArguments(argumentNames, argumentTypes, version, cluster, ksm);
    ...
    private static Map<String, DataType> buildArguments(List<String> names, List<String> types, VersionNumber version, Cluster cluster, KeyspaceMetadata ksm) {
        if (names.isEmpty())
        ...
        for (String name : names) {
            ...
            if (version.getMajor() >= 3) {
                type = DataTypeParser.parse(iterTypes.next(), metadata, ksm.getName(), false);
            } else {
